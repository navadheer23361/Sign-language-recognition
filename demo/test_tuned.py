...
def top_predictions(prediction, labels, top_k=TOP_K):
    if prediction is None:
        return []

    ranked_indices = np.argsort(prediction)[::-1][:top_k]
    return [(labels[i], float(prediction[i])) for i in ranked_indices]
    ...
    try:
        while True:
            ...
            if hands:
                ...
                ranked = top_predictions(prediction, labels)
            ...
            if ranked:
                top_line = " | ".join(f"{label}:{score:.2f}" for label, score in ranked)
                if top_line != last_top_line:
                    print(f"Top-{TOP_K}: {top_line}")
            ...
            cv2.putText(
                img_output,
                f"Threshold: {MIN_CONFIDENCE:.2f} | Window: {WINDOW_SIZE} | Votes: {MIN_VOTES}",
                (20, 112),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (180, 220, 255),
            ...
            for idx, (label, score) in enumerate(ranked, start=1):
                cv2.putText(
                    img_output,
                    f"{idx}. {label}: {score * 100:.1f}%",
                    (20, 112 + idx * 32),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.75,
                    (150, 255, 180) if idx == 1 else (220, 220, 220),
                    2,
                )

            cv2.imshow("Tuned Image", img_output)

            if img_crop is not None:
                cv2.imshow("Tuned ImageCrop", img_crop)
            if img_white is not None:
                cv2.imshow("Tuned ImageWhite", img_white)
